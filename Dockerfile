...
RUN \
    ...
    echo "AREA_DETECTOR=\$(SUPPORT)/areaDetector-R3-8" >>  ${SUPPORT}/configure/RELEASE

WORKDIR ${AREA_DETECTOR}
ADD https://github.com/areaDetector/ADCore/archive/R3-8.tar.gz         ./ADCore.tar.gz
ADD https://github.com/areaDetector/ADSupport/archive/R1-9.tar.gz      ./ADSupport.tar.gz
ADD https://github.com/areaDetector/ADSimDetector/archive/R2-10.tar.gz ./ADSimDetector.tar.gz
...
RUN \
    ...
    ln -s ADCore-R3-8 ADCore && \
    ln -s ADSupport-R1-9 ADSupport && \
    ln -s ADSimDetector-R2-10 ADSimDetector && \
